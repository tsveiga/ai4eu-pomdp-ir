...
class ParseSPUDD {
	...
	IRactNames = new Vector<String>();
	IRactvalNames = new Vector<Vector>();
	actTransitions = new Vector<DD[]>();
	actObserve = new Vector<DD[]>();
	actCosts = new Vector<DD>();
	IRactCosts = new Vector<DD[]>();
	...
	try {
		...
		case StreamTokenizer.TT_WORD:
		    ...
		    else if (stream.sval.compareTo("IRaction") == 0) {
			parseIRAction();
			break;
		    }
		    ...
		    error("Expected \"unnormalized\" or \"dd\" or \"action\" or \"IRaction\" or \"reward\"");
    ...
    public void parseIRAction() {
	
	try {

	    // parse action name
	    stream.nextToken();
	    if (IRactNames.contains(stream.sval)) error("Duplicate action name");
	    else IRactNames.add(stream.sval);	    
	    //IRactCosts.add(DD.zero);
	    DD[] cpts = new DD[nStateVars];
		...
		if (stream.sval.compareTo("endaction") == 0) break;
		...
		else if (stream.sval.compareTo("values") == 0){
		    if (stream.nextToken() != '(') error("Expected '(' before values");
		    Vector<String> varValNames = new Vector<String>();
		    while(true) {
			if(StreamTokenizer.TT_WORD == stream.nextToken()){
			    if (varValNames.contains(stream.sval)) error("Duplicate value name");
		    ...
		    IRactvalNames.add(varValNames);
		}
		
		// cost
		else if (stream.sval.compareTo("cost") == 0) {
		    if (stream.nextToken() != '(') error("Expected '(' before values");
		    DD[] IRcostDD = new DD[IRactvalNames.get(IRactvalNames.size()-1).size()];
		    while(true) {
		    	stream.nextToken();
			if(stream.ttype == ')'){
			    break;
			}		   
		    	int valId = IRactvalNames.get(IRactvalNames.size()-1).indexOf(stream.sval);
		    	if (valId == -1) error("Not an existing IR variable");
		    	IRcostDD[valId] = OP.reorder(parseDD());
		    }
		    IRactCosts.add(IRcostDD);
		}
	    }						
