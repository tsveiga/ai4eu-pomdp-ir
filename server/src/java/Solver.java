...
class Solver implements Serializable {
	...
	if (args.length >= 2) {
		...
		if (ioarg.equals("-i"))  {
			...
			maxBelStates = numBelStates * 1000;
			thearg += 2;
		    } else {
			numargs=0;
		    }
		...
		if (obj instanceof POMDP) {
		    ...
		    */
			...
			while (nits > 0) {
			    ...
			    IRactId = pomdp.IRpolicyQuery(belState); //IR extension
			    System.out.println("action suggested by policy: "+actId+" which is "+pomdp.actions[actId].name);
			    //IR extension
			    System.out.print("IR actions suggested by policy: (");
			    //for (int i=0; i<pomdp.nIRActionsVars; i++) {
			    //	System.out.print(IRactId[i]+",");
			    //}
			    //System.out.println(")");

			    System.out.print(pomdp.IRactions[0].valNames[IRactId[0]]);
			    for (int i=1; i<pomdp.nIRActionsVars; i++) {
				System.out.print(","+pomdp.IRactions[i].valNames[IRactId[i]]);
			    }
			    System.out.println(")");
			    //
